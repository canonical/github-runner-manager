...
def check_active_runner(
    ...
    return all(
        (
            _run_health_check_runner_installed(ssh_conn, instance),
            _run_health_check_cloud_init(ssh_conn, instance.server_name, accept_finished_job),
            _run_health_check_runner_processes_running(ssh_conn, instance.server_name),
        )
    )


@retry(exception=SSHError, tries=3, delay=5, backoff=2, local_logger=logger)
def _get_ssh_connection(openstack_cloud: OpenstackCloud, instance: OpenstackInstance) -> SSHConnection:
...
def _run_health_check_cloud_init(
    ...
    Returns:
        Whether the cloud-init status indicates the run is healthy.
    """
...
def _run_health_check_runner_installed(
    ...
    Returns:
        Whether the runner has been found to be installed.
    """
    result = ssh_conn.run(
...
def _run_health_check_runner_processes_running(ssh_conn: SSHConnection, server_name: str) -> bool:
    ...
    Returns:
        Whether the runner processes have been found running.
    """
